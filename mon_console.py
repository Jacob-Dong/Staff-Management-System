...
from mon_add_panel import mon_add_panel
from mon_query_panel import mon_query_panel
from mon_modify_panel import mon_modify_panel


#建立界面类
class mon_console(QDialog):
    def __init__(self,parent = None):
        super(mon_console,self).__init__(parent)

        #设置界面大小、名称、背景
        self.resize(500,600)
        self.setWindowTitle('月度成就管理')

        #窗体属性
        self.setWindowFlags(Qt.Widget)


        sql="SELECT staId,staName,reTime FROM monthrecord order by reTime"

        #数据列名
        col_lst=['月度优秀员工id','月度优秀员工姓名','评定时间']
        type=1
        ...
        self.addButton = self.buttonBox.addButton("&添加优秀员工",QDialogButtonBox.ActionRole)
        ...
        self.addButton.clicked.connect(self.add_data)#添加优秀员工
    ...
    def query_data(self):
        ...
        self.m_query=mon_query_panel(userid)
        self.m_query.show()
    ...
    def del_data(self):
        ...
        if reply ==  QMessageBox.Yes:
            ...
            ls =int(userid)
            print(ls)
            sql="delete from monthrecord where staId= %s "
    ...
    def modify_data(self):
        ...
        self.m_modify=mon_modify_panel(userid)
        self.m_modify.show()
