...
from staff_good_panel import staff_good_panel


#建立界面类
class staff_good_console(QDialog):
    def __init__(self,parent = None):
        super(staff_good_console,self).__init__(parent)

        #设置界面大小、名称、背景
        self.resize(500,600)
        self.setWindowTitle('优秀员工查看')

        #窗体属性
        self.setWindowFlags(Qt.Widget)


        sql="SELECT staId,staName,staDep FROM staff_good where sta_remarks !='' order by staId"
    ...
    def query_data(self):
        ...
        self.sta_good_query=staff_good_panel(userid)
        self.sta_good_query.show()
    ...
    def del_data(self):
        ...
        if reply ==  QMessageBox.Yes:
            ...
            ls =int(userid)
            print(ls)
            sql="update staff_good set sta_remarks=NULL where staId= %s "
